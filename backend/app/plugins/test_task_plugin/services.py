...
async def calculate_achievement_rate(
    test_mileage: float, actual_mileage: float
) -> Optional[float]:
...
async def get_test_tasks(
    ...
    task_publisher: str = None,
    test_person: str = None,
    task_status: str = None,
    is_kpi: bool = None,
):
    test_start_date, test_end_date = _normalize_date_range(
    ...
    # 处理 is_kpi 筛选,为true时筛选is_kpi == True的，为false时筛选is_kpi == False的
    if is_kpi is not None:
        stmt = stmt.where(models.TestTask.is_kpi == is_kpi)

    total_stmt = select(func.count()).select_from(stmt.subquery())
    total_result = await db.execute(total_stmt)
    total = total_result.scalar_one()

    stmt = stmt.order_by((models.TestTask.id).desc()).offset(skip).limit(limit)
    result = await db.execute(stmt)
    ...
    items = [
        schemas.TestTask(
            ...
            task_publisher=t.task_publisher,
            test_mileage=t.test_mileage,
            test_person=t.test_person,
            actual_mileage=t.actual_mileage,
            task_achievement_rate=t.task_achievement_rate,
            task_status=t.task_status,
            is_kpi=t.is_kpi,
            reason_desc=t.reason_desc,
            remarks=t.remarks,
...
async def update_test_task(
    db: AsyncSession, task_id: int, task: schemas.TestTaskUpdate
):
    stmt = select(models.TestTask).where(models.TestTask.id == task_id)
    result = await db.execute(stmt)
    ...
    for key, value in update_data.items():
        ...
        if key == "project":
            project = value
        elif key == "test_version":
            test_version = value
        elif key == "test_time":
            test_time = value
        elif key == "vin_code":
            vin_code = value
        elif key == "test_function":
            test_function = value
        elif key == "test_mileage":
            test_mileage = value
        elif key == "actual_mileage":
    ...
    return schemas.TestTask(
        ...
        test_version=updated_task.test_version,
        test_time=updated_task.test_time,
        vin_code=updated_task.vin_code,
        test_function=updated_task.test_function,
        task_desc=updated_task.task_desc,
        task_publisher=updated_task.task_publisher,
        test_mileage=updated_task.test_mileage,
        test_person=updated_task.test_person,
        actual_mileage=updated_task.actual_mileage,
        task_achievement_rate=updated_task.task_achievement_rate,
        task_status=updated_task.task_status,
        is_kpi=updated_task.is_kpi,
        reason_desc=updated_task.reason_desc,
...
async def get_daily_task_count(
    ...
    stmt = stmt.group_by(func.date(models.TestTask.created_at)).order_by(
        func.date(models.TestTask.created_at)
    )
...
async def get_task_status_count(
    db: AsyncSession, start_date: str = None, end_date: str = None, project: str = None
):
    """获取任务状态分布"""
    stmt = select(
...
async def get_function_task_count(
    ...
    stmt = stmt.group_by(models.TestTask.test_function).order_by(
        func.count(models.TestTask.id).desc()
    )

    result = await db.execute(stmt)
    return [
        {"function": row.function or "未分类", "count": row.count}
