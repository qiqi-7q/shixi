...
class VehicleBase(BaseModel):
    ...
    parking_location: Optional[str] = Field(None, max_length=200, description="停车地点")
    engine_num: Optional[str] = Field(None, max_length=100, description="驱动电机号/发动机号")
...
class VehicleResponse(VehicleBase):
    """车辆响应模型 - 包含数据库自动生成的字段"""
    id: int
    vehicle_status: VehicleStatus = Field(VehicleStatus.AVAILABLE, description="车辆状态")
...
class BorrowRecordBase(BaseModel):
    ...
    status: Optional[str] = Field(None, max_length=20, description="借用状态：active/returned/cancelled")
    remarks: Optional[str] = Field(None, description="备注")
