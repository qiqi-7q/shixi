...
async def update_test_miles(
    db: AsyncSession, miles_id: int, data: schemas.TestMilesUpdate
):
    stmt = select(models.TestMiles).where(models.TestMiles.id == miles_id)
    result = await db.execute(stmt)
...
async def get_version_mileage(
    ...
    stmt = stmt.group_by(models.TestMiles.test_version).order_by(
        func.sum(models.TestMiles.mileage).desc()
    )

    result = await db.execute(stmt)
    return [
        {
            "version": row.version or "未分类",
...
async def get_daily_mileage(
    ...
    stmt = stmt.group_by(func.date(models.TestMiles.test_start_time)).order_by(
        func.date(models.TestMiles.test_start_time)
    )

    result = await db.execute(stmt)
    return [
        {"date": str(row.date), "total_mileage": float(row.total_mileage or 0)}
        for row in result.all()
    ]
...
async def get_function_mileage(
    ...
    stmt = stmt.group_by(models.TestMiles.test_function).order_by(
        func.sum(models.TestMiles.mileage).desc()
    )
...
async def get_mileage_stats(
    ...
    """获取里程统计数据（包含所有统计信息）"""
    version_mileage = await get_version_mileage(
        db, project, test_version, test_function, start_date, end_date
    )
    daily_mileage = await get_daily_mileage(
        db, start_date, end_date, project, test_version, test_function
    )
    function_mileage = await get_function_mileage(
        db, project, test_version, test_function, start_date, end_date
    )
    overview = await get_mileage_overview(
        db, start_date, end_date, project, test_version, test_function
    )
...
async def get_mileage_overview(
    db: AsyncSession,
    start_date: str = None,
    end_date: str = None,
    project: str = None,
    ...
    stmt = select(
        ...
        func.sum(
            func.IF(
                models.TestMiles.test_function == "NAP", models.TestMiles.mileage, 0
            )
        ).label("nap"),
        func.sum(
            func.IF(
                models.TestMiles.test_function == "CNAP", models.TestMiles.mileage, 0
            )
        ).label("cnap"),
    )
