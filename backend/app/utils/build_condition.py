...
def build_condition(model_class: Type, field_name: str, operator: str, value: Any):
    ...
    if field_name == "test_status" and hasattr(models, "TestStatus"):
        enum_val = getattr(models.TestStatus, value, None) or _find_enum_by_value(models.TestStatus, value)
        if enum_val:
            value = enum_val
    elif field_name == "vehicle_status" and hasattr(models, "VehicleStatus"):
        enum_val = getattr(models.VehicleStatus, value, None) or _find_enum_by_value(models.VehicleStatus, value)
        if enum_val:
            value = enum_val
    elif field_name == "group" and hasattr(models, "VehicleGroup"):
        enum_val = getattr(models.VehicleGroup, value, None) or _find_enum_by_value(models.VehicleGroup, value)
        if enum_val:
            value = enum_val
    elif operator in ("eq", "not_eq") and value is not None and not isinstance(value, list):
